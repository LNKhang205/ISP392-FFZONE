...
const EMPTY_FORM = { name: '', category: 'DRINK', description: '', price: '', imageUrl: '', isActive: true }
...
export default function ServiceManagement() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setPageMsg('❌ ' + getApiError(e, 'Không tải được danh sách dịch vụ'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const openCreate = () => {
    setEditing(null); setForm(EMPTY_FORM); setFormMsg(''); setFormOpen(true)
  }
  ...
  const openEdit = (svc) => {
    ...
    setForm({
      ...
      price: String(svc.price), imageUrl: svc.imageUrl ?? '', isActive: svc.isActive,
    })
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        setPageMsg('✅ Đã cập nhật dịch vụ')
      } else {
        await createService(payload)
        setPageMsg('✅ Đã thêm dịch vụ: ' + form.name.trim())
      }
      closeForm(); await load()
    } catch (e) {
      setFormMsg('❌ ' + getApiError(e, editing ? 'Không cập nhật được' : 'Không thêm được dịch vụ'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleToggle = async (svc) => {
    try {
      ...
      setPageMsg(`✅ Đã ${svc.isActive ? 'tắt' : 'bật'} dịch vụ "${svc.name}"`)
      load()
    } catch (e) {
      alert('Không thể thay đổi trạng thái: ' + getApiError(e))
    }
  }

  const handleDelete = async (svc) => {
    if (!window.confirm(`Xóa dịch vụ "${svc.name}"?\nNếu đã có lịch sử đặt, dịch vụ sẽ bị tắt thay vì xóa.`)) return
    try {
      await deleteService(svc.id)
      setPageMsg('✅ Đã xóa / tắt dịch vụ "' + svc.name + '"')
      load()
    } catch (e) {
  ...
  return (
    <div style={{ padding: '0 4px' }}>
                  ...
                  <td style={{ ...s.td, whiteSpace: 'nowrap' }}>
                    <div style={s.actions}>
                      <button onClick={() => handleToggle(svc)}
                        style={s.btn(svc.isActive ? '#854d0e' : '#166534', svc.isActive ? '#fef9c3' : '#dcfce7', svc.isActive ? '#fde68a' : '#bbf7d0')}>
      ...
      {/* Modal thêm / sửa — giống hệt FieldManagement */}
      {formOpen && (
        <div style={s.overlay}>
          <div style={s.modal}>
            ...
            <div style={s.grid}>
              <div style={s.group}>
                ...
                <input style={s.input} placeholder="VD: Nước suối Aquafina" value={form.name} onChange={set('name')} />
              </div>
              ...
              <div style={{ ...s.group, gridColumn: '1 / -1' }}>
                ...
                <input style={s.input} placeholder="https://... hoặc để trống" value={form.imageUrl} onChange={set('imageUrl')} />
                {form.imageUrl && (
                  <img src={getImgSrc(form.imageUrl)} alt="preview"
              ...
              <div style={{ ...s.group, gridColumn: '1 / -1' }}>
                ...
                <textarea style={{ ...s.input, minHeight: 72, resize: 'vertical' }}
                  placeholder="Mô tả ngắn về dịch vụ..." value={form.description} onChange={set('description')} />
              </div>
            </div>
