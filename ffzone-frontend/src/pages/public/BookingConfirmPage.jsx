...
export default function BookingConfirmPage() {
    ...
    getMyVouchers()
      .then((data) => {
        ...
        const valid = data.filter(
          (v) => !v.isUsed && new Date(v.endDate) > now,
        );
        setMyVouchers(valid);
      })
      .catch(() => setMyVouchers([]))
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all(
      slotIds.map((id) => api.get(`/field-slots/${id}`).then((r) => r.data)),
    )
      .then((results) => {
        ...
        if (taken.length > 0) {
          setError(
            `Một số khung giờ vừa được người khác đặt mất: ${taken.map((s) => fmt(s.startTime)).join(", ")}. Vui lòng quay lại chọn lại.`,
          );
        }
        setSlots(
          results.sort((a, b) =>
      ...
      .catch((err) => {
        console.error(
          "[BookingConfirmPage] Lỗi tải field-slots:",
          err.response?.status,
          err.response?.data || err.message,
        );
        setError(
          `Không thể tải thông tin khung giờ đã chọn. (${err.response?.status || "Lỗi kết nối"})`,
        );
      })
  ...
  const selectedVoucher =
    myVouchers.find((v) => v.id === selectedVoucherId) ?? null;
  const discountAmount = selectedVoucher
    ? selectedVoucher.voucherType === "PERCENT"
      ? Math.round(
          ((fieldAmount + cartTotal) * Number(selectedVoucher.discountValue)) /
            100,
        )
    ...
    try {
      ...
      if (includeCart && cartItems.length > 0) {
        try {
          ...
          setError(
            "Đã tạo đơn đặt sân, nhưng không thể thêm dịch vụ từ giỏ hàng. Bạn có thể thêm lại sau khi thanh toán.",
          );
        }
      }
      ...
      console.error(
        "[BookingConfirmPage] Lỗi tạo booking/payment:",
        err.response?.status,
        err.response?.data || err.message,
      );
      setError(
        err.response?.data?.message ||
          `Không thể tạo đơn đặt sân. (${err.response?.status || "Lỗi kết nối tới server"})`,
      );
      setSubmitting(false);
    }
  };
        ...
        <div className={styles.layout}>
          ...
          <div className={styles.main}>
              ...
              <div className={styles.slotList}>
                {slots.map((s) => (
                  <div key={s.id} className={styles.slotRow}>
                    <span>
                      🕐 {fmt(s.startTime)}–{addMinutes(fmt(s.startTime), 60)}
                    </span>
            ...
            <div className={styles.card}>
              ...
              {cartItems.length === 0 ? (
                <p className={styles.hint}>
                  Chưa có dịch vụ nào. Bạn có thể thêm nước uống, dụng cụ, hoặc
                  tiện ích cho buổi đá.
                </p>
              ) : includeCart ? (
                <div className={styles.slotList}>
                  {cartItems.map((item) => (
                ...
                <p className={styles.hint}>
                  Giỏ hàng sẽ giữ nguyên, bạn có thể thêm dịch vụ sau khi đặt
                  sân.
                </p>
              )}
            ...
            <div className={styles.card}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={styles.voucherRight}>
                          {selected ? (
                            <span className={styles.voucherCheck}>
                              ✓ Đã chọn
                            </span>
                          ) : (
              ...
              {selectedVoucherId && (
                ...
                >
                  ✅ Đã áp mã <strong>{voucherCode}</strong>. Giảm giá sẽ được
                  tính khi thanh toán.
                </p>
              )}
            </div>
          ...
          <div className={styles.summary}>
            ...
            <div className={`${styles.summaryRow} ${styles.totalRow}`}>
              ...
              <span style={selectedVoucher ? { color: "#dc2626" } : {}}>
            ...
            {!selectedVoucher && (
              <p className={styles.hint}>
                💡 Số tiền cuối cùng (sau giảm giá nếu có) sẽ hiển thị trên
                trang thanh toán VNPay.
              </p>
            )}
            <div className={styles.deadlineBox}>
              ⏱️ Bạn có <strong>10 phút</strong> để hoàn tất thanh toán sau khi
              xác nhận.
            </div>
            <button
            ...
            <Link to="/booking" className={styles.backLink}>
              ← Chọn lại khung giờ
            </Link>
          </div>
        </div>
      ...
      {showServiceModal && (
        ...
        >
          <div className={styles.modalBox} onClick={(e) => e.stopPropagation()}>
                  ...
                  return (
                    <div key={svc.id} className={styles.modalServiceRow}>
                      <div className={styles.modalServiceInfo}>
                        <span className={styles.modalServiceName}>
                          {svc.name}
                        </span>
                      ...
                      {inCart ? (
                        <div className={styles.qtyControl}>
                          <button
                            disabled={busy}
                            onClick={() =>
                              handleChangeQty(inCart, inCart.quantity - 1)
                            }
                          >
                            −
                          </button>
            ...
            <button
              className={`btn btn-primary ${styles.modalDoneBtn}`}
              onClick={() => setShowServiceModal(false)}
            >
              Xong
            </button>
