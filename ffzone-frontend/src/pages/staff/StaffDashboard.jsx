...
function CheckIn() {
  ...
  const handleSearch = async () => {
    const q = query.trim().toUpperCase()
    if (!q) return
    setSearching(true); setBooking(null); setSearchErr(''); setActionMsg(null)
    try {
      const r = await api.get(`/bookings/code/${q}`)
      setBooking(r.data)
    } catch (e) {
  ...
  const fmtSlot = (s) =>
    `${s.slotDate}  ${String(s.startTime).substring(0,5)} – ${String(s.endTime).substring(0,5)}`

  const statusCfg = booking ? (BOOKING_STATUS_LABEL[booking.status] || { text: booking.status, bg: '#f3f4f6', color: '#374151' }) : null
  ...
  return (
    <div className={styles.page}>
      ...
      {/* ── Ô tìm kiếm ── */}
      <div className={styles.card}>
        <p className={styles.cardHint}>Nhập mã đặt sân (VD: FFZ-20260620-1234) hoặc scan QR để tìm kiếm:</p>
        <div className={styles.formRow}>
          <input
        ...
        <div className={styles.card}>
          ...
          <div className={styles.infoGrid}>
            ...
            <div className={styles.infoRow}>
              ...
              <div className={styles.slotList}>
                {booking.slots?.map(s => (
                  <span key={s.fieldSlotId} className={styles.slotTag}>{fmtSlot(s)}</span>
                ))}
              </div>
            </div>
          ...
          {/* Action message */}
          {actionMsg && (
            <p className={actionMsg.ok ? styles.successText : styles.errText}>
              {actionMsg.text}
            </p>
          )}
          ...
          <div className={styles.checkinActions}>
            ...
            {booking.status === 'COMPLETED' && (
              <p className={styles.doneText}>🎉 Booking đã hoàn thành.</p>
            )}
            {booking.status === 'CANCELLED' && (
              <p className={styles.errText}>Booking này đã bị hủy, không thể thao tác.</p>
            )}
            {booking.status === 'PENDING_PAYMENT' && (
              <p className={styles.errText}>Booking chưa thanh toán, chưa thể check-in.</p>
            )}
          </div>
        </div>
      )}
